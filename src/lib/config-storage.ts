 ...
 */
export async function saveConfig(config: StoredConfiguration): Promise<StoredConfiguration | null> {
  if (!USE_SERVER_STORAGE) {
    return null;
  }
  ...
  try {
    ...
    await fs.writeFile(tempFile, JSON.stringify(configWithMeta, null, 2), 'utf-8');
 ...
 */
export async function importConfig(jsonData: string): Promise<StoredConfiguration | null> {
  try {
...
export function getEnvDefaultConfig(): Partial<StoredConfiguration> {
  ...
  if (process.env.ADDITIONAL_INBOXES) {
    try {
      const additional = JSON.parse(process.env.ADDITIONAL_INBOXES) as AgentInbox[];
